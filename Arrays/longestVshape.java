...
class Solution {
    ...
    private int dfs(int i, int j, int k, int canTurn, int target, int[][] grid, int[][][] memo) {
        ...
        int res = dfs(i, j, k, canTurn, 3 - target, grid, memo);

        if (canTurn == 1) {
            for (int newK = 0; newK < 4; newK++) {
                if (newK == k)
                    continue;
                res = Math.max(res, dfs(i, j, newK, 0, 3 - target, grid, memo));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sol.lenOfVDiagonal(grid)); // sample output
    }
}
